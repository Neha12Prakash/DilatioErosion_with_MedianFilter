...
__local float device_input[IMAGE_SIZE_X][IMAGESIZE_Y];
__local float device_temp[IMAGE_SIZE_X][IMAGESIZE_Y];
__local float device_output[IMAGE_SIZE_X][IMAGESIZE_Y];
...
void erode(__global const float *d_input, __global float *d_output, __global int *struct_kernel, int sizeX, int sizeY) {
...
void erode_image( __read_only image2d_t d_input,  __read_write image2d_t d_output, __global int *struct_kernel, int sizeX, int sizeY) {
    size_t countX = get_global_size(0);
    size_t countY = get_global_size(1);
    size_t i = get_global_id(0);
    size_t j = get_global_id(1);
...
void dilate(__global const float *d_input, __global float *d_output, __global int *struct_kernel, int sizeX, int sizeY) {
    size_t countX = get_global_size(0);
    size_t countY = get_global_size(1);
    size_t i = get_global_id(0);
    size_t j = get_global_id(1);
    ...
    if (hit){
        d_output[getIndexGlobal(countX, i, j)] = 1;
    }
    else{
        d_output[getIndexGlobal(countX, i, j)] = getValueGlobal(d_input, countX, countY, i, j);
    }

}

const sampler_t dilation_sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST ;
void dilate_image( __read_only image2d_t d_input,  __read_write image2d_t d_output, __global int *struct_kernel, int sizeX, int sizeY) {
    size_t countX = get_global_size(0);
    size_t countY = get_global_size(1);
    size_t i = get_global_id(0);
    size_t j = get_global_id(1);
...
__kernel void opening(              // First dilate then Erode
    // __global const float *d_input, 
    // __global float *d_output, 
    // __global int *struct_kernel, 
    // int sizeX, int sizeY
    ){
